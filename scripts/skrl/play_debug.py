...
from FRE25_IsaacLabSym.utils.DebugPlotter import DebugPlotter
...
def main():
    ...
    obs, _ = env.reset()
    timestep = 0
    totalRewards = torch.zeros((env.num_envs, 1), device=env.device)
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            finalTotalRewards = totalRewards[terminations | truncations]

            if finalTotalRewards.numel() > 0:
                plotter.add_total_reward(finalTotalRewards)
                plotter.plot()

            totalRewards[terminations | truncations] = 0.0
